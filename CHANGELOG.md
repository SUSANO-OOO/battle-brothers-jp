...
- Persisted the Final Producer Project Goal in the native Codex goal facility and repository documents while preserving all 6,275 reviewed canonical units and ignored draft/review evidence.
- Audited the 2026-08-31 public Battle Brothers ecosystem and accepted an evidence-based one-ZIP conditional architecture: Modern Hooks is the only JP Core framework dependency; Legends/MSU/DLC integration is optional; Rosetta/stdlib will not be user-installed JP dependencies.
- Classified the official Legends 19.4.20→19.4.21 delta without changing the verified target: 11 semantic files remain after normalization, with unchanged reviewed assets marked for reuse and changed translation/boundary work isolated before any support claim.
- Traced the onPrepareVariables replacement-key defect through extraction, global deduplication, category selection, and batch construction. Automatic internal classification now requires parser-bound role/provenance design; ambiguous and mixed cases remain review-required pending implementation.
- Kept the final Vanilla literal tranche out of canonical data after an independent real-Win32 retained-handle race reproduced possible external-byte loss; Gate B remains closed until transaction-lifetime writer protection and the same adversarial review pass.
- Closed Gate B through a transaction-lifetime Win32 preimage guard and independent adversarial re-audit. Authorized application split two exact display contexts and resolved 363 reviewed internal-key occurrences across 217 removed units without touching the actual game environment.
- Adopted 140 independently reviewed direct display translations from the final 357-unit Vanilla event literal tranche, bringing canonical REVIEWED coverage to 6,417 with zero translated-needs-review units.
- Corrected one independently detected Devanagari word in a jousting event to `試合場`; placeholder/tag/newline signatures remain unchanged and required font glyph coverage returned to zero missing glyphs.
- Closed all six public ecosystem/ADR review findings, including mutation-resistant exact Legends release fingerprints, while keeping `19.4.21` explicitly unsupported and runtime-unverified.
- Built the reviewed-6,417 development artifact and passed all 32 static/archive QA checks; it remains a non-release artifact while coverage and runtime migration gates are open.
- Replaced the replacement-key heuristic pipeline with independently reviewed `bbjp-squirrel-role-v2` source-bound, all-occurrence classification. Schema-v2 mutators now reject stale source/placeholder evidence, duplicate canonical identity, translation-unit membership drift, parameter shadowing, and non-dominating control flow; the 6,417 reviewed canonical units and all three canonical hashes remain unchanged.
